...
export class Sky {
	...
	public sunDirectionalLight: THREE.DirectionalLight;
	private settings: any;
	private uniforms: any;
	public distance = 400000;
	...
	private createMesh() {
		let skyShader = THREE.ShaderLib[ "sky" ];
		...
		let skyGeo = new THREE.SphereBufferGeometry( 2000, 32, 15 );
	...
	private initializeSettings() {
		this.settings = {
			...
			mieDirectionalG: 0.8,
			luminance: 1,
			inclination: 0.00, // elevation / inclination
	...
	public update(delta?: number, center?: THREE.Vector3) {
		delta = delta || 1;
		if(center) this.mesh.position.copy(center);
